...
- Набор библиотек под конкретные проекты формировать в файле `vcpkg.json` в
папке externals репозитория.
...
- Создать ответвление сборок для конкретного проекта xxx (`externals.xxx`) от
ветки **externals** и добавить в файл externals/vcpkg.json необходимые
библиотеки.
...
> сделать clean up в папке репозитория.
...
библиотеки ОГРОМНОГО размера, поэтому изменены настройки в файое windows.cmake.
